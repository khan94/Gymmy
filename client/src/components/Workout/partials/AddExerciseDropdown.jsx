...
const tempExerciseList = ['Bench Press', 'Dumbbell Press', 'Arnold Shoulder Press']
...
const AddExerciseDropdown = ({ isOpen, setIsOpen, handleCreateNewExercise }) => {
  ...
  const getExercises = useCallback(async () => {
    try {
      ...
      //   headers: { 'X-Api-Key': import.meta.env.VITE_EXERCISES_API_KEY },
      // })
      const res = [
        {
          name: 'Rickshaw Carry',
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      // setOptions(tempExerciseList.filter((ex) => ex.toLowerCase().includes(searchValue.toLocaleLowerCase())))
    }
  }, [searchValue])
