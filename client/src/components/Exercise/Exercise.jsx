import { faCaretDown, faCaretUp, faEdit, faInfoCircle, faTrash } from '@fortawesome/free-solid-svg-icons'
...
const Exercise = ({ exercise, isExpanded, setExpandedExercise, handleAddSet, handleUpdateSet, getWorkoutDetails }) => {
  ...
  const handleDeleteExercise = async () => {
    try {
      const res = await axios.delete(`${import.meta.env.VITE_CORE_API}/api/exercises/${exercise.id}`)
  ...
  const handleDeleteSet = async (setId) => {
    try {
      const res = await axios.delete(`${import.meta.env.VITE_CORE_API}/api/sets/${setId}`)
  ...
  return (
    <div className="border border-zinc-500 p-2 rounded-xl">
      <div
        ...
        onClick={() => (isExpanded ? setExpandedExercise(null) : setExpandedExercise(exercise.id))}
      >
            ...
            exercise?.sets?.map((set, i) => (
              <div key={set.id} className="  py-1">
                <div className="flex flex-row justify-between items-center">
                  <span className="font-extrabold">SET {exercise.sets.length - i}:</span>
                  <span>{set.weight} lbs</span>
                  <span>{set.reps} reps</span>
                  <div className="flex flex-row gap-5">
                    <FontAwesomeIcon role="button" icon={faTrash} onClick={() => setSetIdToDelete(set.id)} />
                    <FontAwesomeIcon role="button" icon={faEdit} onClick={() => setEditingSet(set)} />
                  </div>
                </div>
      ...
      {Boolean(editingSet) && (
        <EditSetModal editingSet={editingSet} handleUpdateSet={handleUpdateSet} onClose={() => setEditingSet(null)} />
      )}
      {isDeleteExerciseModalOpen && (
        ...
        >
          <div>
            Are you sure you want to delete exercise <span className="font-extrabold">{exercise.name}</span> with
            <span className="font-extrabold"> {exercise?.sets.length} sets</span>?
          </div>
        </ConfirmDeleteModal>
      )}
      {setIdToDelete && (
      ...
      {isInfoModalOpen && <ExerciseInfoModal exercise={exercise} onClose={() => setIsInfoModalOpen(false)} />}
    </div>
  )
}
